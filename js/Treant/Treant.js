...
;( function() {
    ...
    Tree.prototype = {
        ...
        positionNodes: function() {
            ...
            for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
                ...
                // Nitin: Added support for node level orientation to support horizontal connectos (such as, for spouse relationship)
                if (node.orientation) {
                    var space = self.CONFIG.siblingSeparation;
                    var parentNode = node.parent();
                    node.X = parentNode.X + parentNode.width + space;
                    node.Y = parentNode.Y;
                    var deltaWidth = (node.width + space);
                    if ((node.X + deltaWidth) > treeSize.x.max) {
                        treeSize.x.max += deltaWidth;
                        treeWidth = treeSize.x.max - treeSize.x.min;
                        treeCenter.x = treeSize.x.max - treeWidth/2;
                        this.handleOverflow(treeWidth, treeHeight);
                        deltaX = containerCenter.x - treeCenter.x;
                        negOffsetX = ((treeSize.x.min + deltaX) <= 0) ? Math.abs(treeSize.x.min) : 0;
                    }
                ...
                // Nitin: Do the following only for original flow as it will otherwise overwrite the (X, Y) computed values above.
                if (!node.orientation) {
        ...
        getPathString: function( from_node, to_node, stacked ) {
            ...
            // Nitin: Adjust the connector points to show the horizontal connector
            if (to_node.orientation == 'EAST') {
                startPoint.x = from_node.X + from_node.width;
                endPoint.x = to_node.X;
                endPoint.y = to_node.Y + (to_node.height / 2);
    ...
    TreeNode.prototype = {
        ...
        reset: function( nodeStructure, id, parentId, tree, stackParentId ) {
            ...
            // Nitin: Added support for node level orientation to support horizontal connectos (such as, for spouse relationship)
            this.orientation = nodeStructure.orientation ? nodeStructure.orientation : null;
            // Nitin: Change ends
    ...
    TreeNode.prototype.createGeometry = function( tree ) {
        ...
        // Nitin: Added support for setting the anchor 'id' dynamically via the link node structure.
        if (this.link.id) {
          node.id = this.link.id;
        }
        // Nitin: Change ends
        else if ( this.nodeHTMLid ) {
